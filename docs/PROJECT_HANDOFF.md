...
- **정지(idle)/차량(vehicle)은 감점하지 않는다 - 오직 확정 보행 중 사용만 감점(핵심 결함 수정)**: 예전 백그라운드 파이프라인은 "차량만 아니면 전부 보행"으로 취급했다(옛 `shouldSkipAsVehicle` 가 `vehicle` 일 때만 스킵하고, `idle`/`walking` 은 모두 `walking=true` 로 흘려보냄). 그래서 **가만히 서서(idle, 속도≈0) 폰을 켜놓기만 해도** pitch>=10 3초 지속이 '사용'으로 잡혀 감점되고, 정지 시간이 use 로 누적되며 usageRatio 페널티까지 겹쳐 **점수가 빠르게 떨어지는** 증상(사용자 보고: '정지해서 켜놓기만 했는데 점수가 계속 떨어짐', '대중교통 이용 시 감점')이 났다. 이제 `src/sensors/motionClassifier.ts#decideLocationWalking`(순수)이 `classifyMotion` 의 walking/idle/vehicle 를 그대로 얻어 각 경로가 무엇을 할지 결정하고, `backgroundTask.ts` 의 두 경로(연속 위치 태스크 핸들러 + 지오펜스 콜백)가 그 결과로 `processLocationSample` 을 호출한다.
  ...
  - **속도 결측 처리 규칙(정직성)**: 백그라운드 위치 샘플의 `speed` 는 null/undefined/음수/비유한(미측정)일 수 있다. 예전에는 결측 시 분류를 건너뛰고 `walking=true` 로 하드코딩해, **확정되지 않은 보행이 감점**될 수 있었다. 이제 속도 결측 시 보행을 단정하지 않는다: 히스테리시스로 **직전 분류가 walking 이면** 단발 결측 1샘플은 walking 을 유지(정상 보행의 과소 계상 완화), **아니면 walking=false** 로 보수 처리한다. 결측이 지속되면 자이로의 walking 신선도(`WALKING_SIGNAL_STALE_MS=8초`)가 만료돼 결국 no-use 로 수렴한다. 즉 **과소 계상 방향으로만 치우치며, 확정되지 않은 보행으로 사용 감점을 하지 않는다.** 속도 결측 샘플로는 분류기 상태(쿨다운/걸음 기준)를 흔들지 않는다(`next=prev`).
  - `processLocationSample` 의 `walking` 파라미터 안전 기본값은 `false`(정직성). 두 호출부 모두 `decideLocationWalking` 로 walking 을 명시적으로 계산해 넘긴다. 순수 로직은 `scripts/verify-location-walking.ts`(정지=no-use/보행=use/차량=스킵/속도결측 히스테리시스, mutation-sensitive)로 검증한다.
  - **진단 반영**: 진단 패널(`app/diagnostics.tsx` ← `readPostureDiagnostics`)의 walking 근거 문구가 `walking(보행)`/`idle(정지) - 감점 안 함`/`vehicle(차량) - 감점 안 함`을 구분해 보여준다.
- **리포트 '위험' 등급 문구 정직화**: 리포트 상단 피드백 카드의 '위험' 등급은 **점수가 기준선(60) 미만**이라는 뜻이지, 사용자가 지금 위험 구역(사고다발구역) 안에 있다는 뜻이 아니다. 예전 문구('위험 구간입니다')는 위험 구역 밖에서도 현재 위치가 위험 구역인 것처럼 오해를 줬다(사용자 지적). 점수 기반 안내 문구로 바로잡았다(`app/report.tsx` `GRADE_FEEDBACK.danger`).
