...
App Store 제출에는 실제 아이콘·스플래시 에셋(PNG 바이너리)이 필요합니다. 현재 리포지토리에는 최종 PNG 에셋이 포함되어 있지 않으며, 재디자인한 **SVG 시안** 중 **시안 B 가 채택**되었습니다.

> ✅ **시안 B 채택됨.** `assets/icon.svg` / `assets/splash.svg` 가 정식본입니다(시안 B — 딥 틸/청록 `#0f3b45`→`#14515e`, 레이더 동심 호 + 현재위치 도트). PNG 변환 시 **배경 불투명 `#0f3b45`, 아이콘 1024×1024, 알파(투명) 끄기**. 변환 후 `assets/icon.png` / `assets/splash.png` 로 저장하고 `app.json` 에 연결하세요. 지금은 PNG 가 없으므로 `app.json` 미연결(빌드 안전) 상태를 유지합니다.

- **정식 아이콘**: `assets/icon.svg` (1024×1024, 불투명 배경).
- **정식 스플래시**: `assets/splash.svg` (1242×2688 권장, 동일 톤).
- 선택되지 않은 시안: `assets/icon-candidates/icon-a.svg` · `icon-c.svg`, `assets/splash-candidates/splash-a.svg` · `splash-c.svg` 에 보관(참고용).
...
| 스플래시 이미지 | `assets/splash.png` | 최소 1242×2688 권장, 단색 배경(#0f3b45) 위 중앙 로고 |
| Android 적응형 아이콘 foreground | `assets/adaptive-icon.png` | 1024×1024, 안전 영역 고려한 여백 |

## PNG 만드는 방법 (시안 SVG 활용)

> **알파(투명도) 제거가 핵심**: App Store 아이콘은 **알파 채널이 없어야** 심사를 통과합니다. 배경을 불투명하게 유지하고, 내보낼 때 투명 배경 옵션을 끄세요. (시안 SVG 는 이미 불투명 배경으로 만들어져 있습니다.) 채택된 시안 B 의 배경색은 **`#0f3b45`** 입니다. 정식본 `assets/icon.svg` / `assets/splash.svg` 를 사용하세요.
...
```bash
# 아이콘 (1024x1024, 불투명 배경 위 렌더) — 시안 B (#0f3b45)
...
- `-b "#0f3b45"` 로 배경을 불투명하게 채워 알파를 제거합니다.
...
실제 PNG 를 넣은 뒤 `app.json` 의 `expo` 블록에 아래를 추가합니다(`backgroundColor` 는 채택된 시안 B `#0f3b45`). **지금은 PNG 가 없으므로 이 연결을 추가하지 마세요 — 없는 PNG 를 참조하면 빌드가 실패합니다.**
...
- **현재 상태**: **시안 B 채택 완료** — 정식본 `assets/icon.svg` / `assets/splash.svg` 존재. 최종 PNG 는 아직 없음. `app.json` 에 icon/splash 미연결(빌드 안전).
- **왜 PNG 가 없나**: 샌드박스에 SVG→PNG 변환 도구가 없고 외부망 차단으로 설치 불가.
- **제출 전 필수(사용자 로컬)**: 위 방법 A/B/C 중 하나로 `assets/icon.svg` / `assets/splash.svg` 를 PNG 생성(배경 `#0f3b45`, 알파 끄기) → `assets/icon.png` / `assets/splash.png` 로 추가 → `app.json` 에 경로 연결(`backgroundColor` `#0f3b45`) → EAS 빌드로 확인.
