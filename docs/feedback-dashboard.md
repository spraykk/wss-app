...
- `public.feedback` 테이블 (RLS on, anon insert 정책만). 익명 연령대 밴드(`age_band`, 선택) 컬럼 포함.
- `list_feedback()` RPC — 원시 `device_id` 대신 익명 참조코드로 목록 반환(최신순). 연령대 밴드(`age_band`)도 함께 반환(미선택은 null).
- `get_feedback_summary()` RPC — 총건수·카테고리별·참여 기기수·평균 별점 요약
- `get_feedback_summary_by_age()` RPC — 연령대 밴드별 건수·평균 별점·참여 기기수(미선택은 '미상'으로 집계, 합계는 전체와 동일)

위 RPC 들 모두 anon 역할에 execute 권한이 부여됩니다. 원시 행 select/update/delete 는
정책이 없어 차단되므로, 공개(anon) 키로도 개별 원시 데이터에는 접근할 수 없습니다.
연령대는 정확한 나이가 아니라 5개 밴드(10대/20대/30대/40대/50대+) 중 하나이며, 온보딩에서
1회 선택한 값이 자동으로 첨부됩니다(미선택은 '미상'). 원시 `device_id` 는 여전히 노출되지 않습니다.
...
- 상단 요약(총 N건 · 참여자 M명 · 평균 별점 · 카테고리 분포 · 연령대 분포)과 피드백 카드
  목록이 한 페이지에 보기 좋게 나옵니다. 카테고리 필터로 원하는 종류만 추려 캡처할 수 있습니다.
  각 카드에는 작성자의 연령대 밴드(미선택은 '미상')가 익명 참조코드와 함께 표시됩니다.
...
- 원시 `device_id`, 위치·경로 등은 절대 표시되지 않습니다(애초에 수집하지 않거나 RPC가
  익명 참조코드로 대체). 연령대는 정확한 나이가 아니라 밴드(범위)만 표시되며 개인을 특정하지 않습니다.
